...
def _bash_bin() -> str:
    ...
    """
    if os.name == "nt":
        candidates = [os.environ.get("JOBFINDEROS_BASH_BIN", "")]
        program_files = os.environ.get("ProgramFiles", "")
        candidates += [str(Path(program_files) / "Git" / "bin" / "bash.exe")]
        candidates += [str(Path(program_files) / "Git" / "usr" / "bin" / "bash.exe")]
...
def main() -> None:
    ...
    try:
        cmd = [_bash_bin(), 'scripts/JobFinderOS_run_skill.sh', 'jobs-daily', 'jobs-daily']
